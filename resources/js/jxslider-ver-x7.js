...
1. nOnMargin 옵션 추가
on 클래스가 들어간 부분의 마진을 다르게 주기 위한 옵션(* 중요 )

2. addEventMoment 수정
클래스를 on을 바로 넣은경우는 다르게 준 마진값을 다시 변경해야한다.

3. update( 1 or 2 ) 의 경우를 적용하기 위해 아래 코드와 위에 몇줄 코드를 함께 위로 올렸다.
이유는 처음 플러그인이 시작할때 on을 넣기 위한 코드가 있는데(아래코드) 이걸 먼저 정하고 나서
nMargin , nOnMargin 을 객체 상황에 맞춰 넣기 위해서 코드를 위로 올렸다.
...
~~~~~생략
_this.jxwrap.find(">.jx-unit").eq( _this.defaults.nBackCopy ).addClass("on"); 까지 코드를 위로 올렸다.


4.  addEnterFrame 함수 수정
oEffect 옵션을 넣었을때만 작동하도록 수정을 하였다.
리사이징을 안할때는 브라우저 창을 움직이면 문제가 발생하여서 아래와 같이 수정
_this.nCenter = _this.jxwrap.offset().left + _this.nBase; 이것을
_this.nCenter =  _this.defaults.bResize ? _this.jxwrap.offset().left + _this.nBase : _this.jxwrap.position().left + _this.nBase ; 수정

var left = $(this).offset().left; 이것을 아래와 같이 수정
var left = _this.defaults.bResize ? $(this).offset().left : $(this).position().left
...
JXSLIDER.prototype.resize = function()
{
    ...
    if ( _this.defaults.sDirection == "horizontal")
    {
        ...
        this.jxwrap.find(">.jx-unit").each(function(i, e)
        {
            if ( $(this).hasClass('on') )
            {
                $(this).css(  {"width": Math.ceil( _this.jxbox.innerWidth() / _this.defaults.nView ) - _this.defaults.nMargin ,
                                "margin-left": _this.defaults.nOnMargin - _this.defaults.nMargin , "margin-right": _this.defaults.nOnMargin } );
            }
            else
            {
    ...
    else
    {
        ...
        this.jxwrap.find(">.jx-unit").each(function(i, e)
        {
            if ( $(this).hasClass('on') )
            {
                $(this).css(  { "height": Math.ceil( _this.jxbox.innerHeight() / _this.defaults.nView )-_this.defaults.nMargin ,
                    "float": "none", "margin-top": _this.defaults.nOnMargin - _this.defaults.nMargin , "margin-bottom": _this.defaults.nOnMargin } );
            }
            else
            {
                $(this).css(  {"height": Math.ceil( _this.jxbox.innerHeight() / _this.defaults.nView )-_this.defaults.nMargin ,
                    "float": "none", "margin-bottom": _this.defaults.nMargin} );
            }
