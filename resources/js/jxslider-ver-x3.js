...
bug :
1 . addEventAction 함수 수정
왼쪽 객체를  클릭했을때 우쪽으로 움직이는 _this.lcal( Math.abs(nChr) ); 가 빠져있어서 추가를 하였다.

update :

1 . 좌우 버튼 클릭할때만 on을 바로 적용하도록 수정
JXSLIDER.prototype.addEventMoment 추가

2. 좌우 터치 드래그 할때 바로 효과를 적용
sEffectClass : 어떤 오브젝트를 효과를 적용할것인지
어떤 효과를 줄것인지를 배열고 넣었다.
oEffect :
[
    {
        name : 'transform' ,
