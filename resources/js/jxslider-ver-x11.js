...
추가 :
oEffect 효과 공식 수정과 함께 bEffBalnce: true 옵션을 추가하여 가운데 객체를 기준으로 양쪽의 비율을 같이 할것인지 서서히 변화를 줄것인지에 대한 옵션을 추가하였다.
*/
...
function JXSLIDER(obj, option) {
  ...
  this.defaults = {
    ...
    //sEffectClass : 'thumb' ,
    bEffBalnce: true, // 양쪽 객체의 효과를 거리에 따라 비율에 맞춰 투명도, 각도, 크기를 변화준다(기본값 : true), false값은 양쪽 객체들이 min값에 맞춰서 똑같이 적용.
    oEffect: [
      /*
      {
          name : 'transform' ,
...
JXSLIDER.prototype.addEnterFrame = function() {
  ...
  */
  ...
  var sClass = $.trim(_this.defaults.sEffectClass); //글자 공백 제거
  //sCalss = 'thumb';
  var cuv = 1;
  var sEffectName = '';
    ...
    _this.jxwrap.find(">.jx-unit").each(function(i, e) {
      ...
      for (j in _this.defaults.oEffect) {
        ...
        if (name == 'opacity') {
          ...
          effectObj.css({ 'opacity': ratio(min,max,distance) });
          ...
          for (m in _this.defaults.oEffect[j].ef) {
            ...
            if (name == 'scale') {
              sEffectName += name + '(' + ratio(min,max,distance) + ')';
            } else if (name == 'rotateY') {
              sEffectName += name + '(' + (ratio(min,max,distance) * cuv) + 'deg)  ';
            }
  ...
  function ratio(min,max,distance) {
    var tar;
    if ( _this.defaults.bEffBalnce ) {
      tar = (min - max) / (0 - (_this.nMove*_this.defaults.nView/2) + (_this.nMove/2) ) * (0 - Math.abs(distance)) + max;
    } else {
      tar = (min - max) / (0 - _this.nMove ) * (0 - Math.abs(distance)) + max; //양쪽이 min값에 같게
      if (max >= min) {
        if (tar <= min) {tar = min; }
      } else {
        if (tar >= min) { tar = min; }
